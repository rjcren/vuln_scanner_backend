...
class AWVS:
    api_base_url = os.getenv("AWVS_API_URL", "https://127.0.0.1:3443")
    _api_key = os.getenv("AWVS_API_KEY")
    auth_headers = {
        "X-Auth": _api_key,
    ...
    targets_api = f"{api_base_url}/api/v1/targets"
    scan_api = f"{api_base_url}/api/v1/scans"
    report_api = f"{api_base_url}/api/v1/reports"
    # vuln_api = f"{api_base_url}/api/v1/vulnerabilities"
    # create_group_api = f"{api_base_url}/api/v1/target_groups"
    ...
    @staticmethod
    def add_url(task_id, url):
        """添加url到AWVS"""
        try:
            data = {"address": url, "description": "自动化添加，请勿删除"}
            res = requests.post(AWVS.targets_api, headers=AWVS.auth_headers, json=data, verify=False)
            if res.status_code == 201:
                TaskLog.add_log(task_id, "INFO", "AWVS添加url成功")
                return res.json()["target_id"]
            else:
                error_message = f"HTTP {res.status_code}: {res.text}"
                TaskLog.add_log(task_id, "ERROR", f"AWVS添加url失败: {error_message}")
        except AppException:
            raise
    ...
    @staticmethod
    def start_scan(task_id, target_id, profile_key="full_scan"):
        """运行AWVS扫描任务"""
        data = {
            "target_id": target_id,
            "profile_id": AWVS.profile_dict.get(profile_key),
            "schedule": {
                "disable": False,
                "start_date": None,
        ...
        try:
            res = requests.post(AWVS.scan_api, json=data, headers=AWVS.auth_headers, verify=False)
            # logger.error(data)
            if res.status_code == 201:  
    ...
    @staticmethod
    def delete(scan_id):
        target_id = AWVS.get_scan(scan_id).get("target_id")
        delete_api = f'{AWVS.targets_api}/{target_id}'
        try:
            requests.delete(delete_api, headers=AWVS.auth_headers, verify=False)
        except Exception as e:
            raise InternalServerError(f"删除扫描任务失败{str(e)}")

    @staticmethod
    def get_scan(scan_id: str):
        try:
            res = requests.get(url=f"{AWVS.scan_api}/{scan_id}", headers=AWVS.auth_headers, verify=False)
            return res.json()
        except Exception as e:
            raise InternalServerError(f"获取扫描任务失败{str(e)}")

    @staticmethod
    def get_vuls(scan_id, scan_session_id):
        """获取任务漏洞列表"""
        scan_result_api = f'{AWVS.scan_api}/{scan_id}/results/{scan_session_id}/vulnerabilities'
        try:
            response = requests.get(scan_result_api, headers=AWVS.auth_headers, verify=False)
    ...
    @staticmethod
    def get_vuln_detail(scan_id, scan_session_id, vuln_id):
        """获取任务中漏洞具体的漏洞信息"""
        scan_vuln_detail_api = f'{AWVS.scan_api}/{scan_id}/results/{scan_session_id}/vulnerabilities/{vuln_id}'
        try:
            response = requests.get(scan_vuln_detail_api, headers=AWVS.auth_headers, verify=False)
            return response.json()
        except Exception as e:
            raise InternalServerError(f"获取漏洞列表失败: {str(e)}")
        
    @staticmethod
    def get_vuln_statistics(scan_id, session_id):
        """获取漏洞概述，主要用来获取漏洞时间"""
        scan_vuln_statistics_api = f'{AWVS.scan_api}/{scan_id}/results/{session_id}/statistics'
        try:
            response = requests.get(scan_vuln_statistics_api, headers=AWVS.auth_headers, verify=False)
            return response.json()
        except Exception as e:
            raise InternalServerError(f"获取漏洞概述失败: {str(e)}")

    @staticmethod
    def save_vuls(task_id, scan_id):
        """AWVS获取指定任务的漏洞"""
        try:
            res = AWVS.get_scan(scan_id)
            print(f"AWVS漏洞详情: {res}")
            progress = res.get("current_session", {}).get("progress")
            session_id = res.get("current_session", {}).get("scan_session_id")
            if not session_id:  # 因AWVS任务状态类型颇多，此处会出现scan_session_id返回值为空的情况，故请求到有为止
                return False
            # 获取扫描的结果列表
            vuln_list = AWVS.get_vuls(scan_id, session_id)
            if vuln_list:
                vul_statistics = AWVS.get_vuln_statistics(scan_id, session_id)
                statisticses = vul_statistics.get("scanning_app").get("wvs").get("main").get("vulns")
                vul_detail_list = []
                map = {
                    "0": "info",
                    "1": "low",
                    ...
                    try:
                        ...
                        for index, sta in enumerate(statisticses):
                            if vul.get("vuln_id") == sta.get("vuln_id"):
                                num = index
                                break
                        vul_detail = AWVS.get_vuln_detail(scan_id, session_id, vul.get('vuln_id'))
                        vul_detail_list.append(Vulnerability(
                            scan_id = vul.get("vuln_id"),
                            scan_source =  "AWVS",
                            vul_type = vul_detail.get("vt_name"),
                            severity = map.get(str(vul_detail.get("severity")), "info"),
                            description = vul_detail.get("description"),
                            details = vul_detail.get("details"),
                            solution = vul_detail.get("recommendation"),
                            time = datetime.fromisoformat(statisticses[num].get("time"))
                        ))
                    except Exception as e:
